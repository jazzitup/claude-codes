...
description: "사용자의 Evernote에서 'weekly todo's' 계열 노트(제목이 'June 4', 'July 3', 'August 2'처럼 <월 이름> <주차번호>인 노트)를 새 주차용으로 만든다. 전용 템플렛 노트(제목이 'weekly todo template ...'이고 attributes.isTemplate:true)를 찾아 표 구조를 그대로 옮기고, 대상 주의 일~토 날짜를 표 안에 채워 넣는다. 2026-08-24부터 표는 2개(Routine 표 + Motto/Urgent items 표)로 간소화됐다 — 매일 오늘 칸(공복 유지/공부 시간/조깅)을 채우는 건 별도 [[evernote-daily-checkin]] 스킬이 세션 시작마다 자동으로 한다. 지난 15일간의 메일(Gmail + 로컬 Mail.app의 sejong.ac.kr/CERN 계정)을 훑어 대상 주에 마감·일정이 있는 항목을 Urgent items 체크리스트에 자동으로 추가한다. Evernote MCP(https://mcp.evernote.com/mcp)가 로컬 Claude Code 세션에 연결되어 있으면 실제로 노트를 생성/수정한다 — 연결 상태·설정 방법은 10절 참고. 매주 금요일 오전 9시(KST)에 claude.ai 예약 routine으로도 자동 실행된다(단, routine은 Gmail만 스캔 가능하고, 별도로 클라우드 커넥터에도 Evernote가 연결되어 있어야 함 — 10절). '이번 주 위클리 투두 노트 만들어줘', '<월> <N>주차 노트 만들어', 'weekly todo 다음 주 걸로 만들어줘', 'X월 X일 템플렛 써서 넣어줘', '메일 확인해서 리마인더 넣어줘' 요청 시 사용."
---
...
**2026-08-24 이후 형식(간소화됨)**: 주간 노트 본문은 표 **2개** +
구분용 `<div><br/></div><div><br/></div>`로 이루어진다. (이전에는 표
3개였다 — 가운데 "Week / Morning Study / Jogging / Night writing" 빈 표는
사용자가 2026-08-24에 없앴다. 그 표를 아직 없앤 적 없는 아주 옛날 노트가
있을 수 있으나, **새로 만드는 노트는 항상 아래 2-표 형식**을 따른다.)

1. **Routine 표** — 8칸(빈 라벨 칸 + Sun~Sat) 헤더 다음에 항목별로
   한 행씩 온다(요일별이 아니라 **항목별** 행이다):
   - `전날 밤 공복 유지` 행 — 7개 요일 칸, 값은 `O`(했음)/`X`(안 했음)
   - `아침 공부 시간` 행 — 7개 요일 칸, 값은 `시:분` 형식(예: `1:30`)
   - `밤 조깅` 행 — 7개 요일 칸, 값은 `O`/`X`

   **날짜를 넣지 않는다** — 이 표는 그 주가 진행되면서 매일 채워지는 표다
   (자동 채움 대상). 새 주 노트를 만들 때는 템플렛에서 그대로 빈 칸으로
   복사한다. 매일 오늘 칸을 채우는 건 이 스킬이 아니라
   [[evernote-daily-checkin]] 스킬의 역할이다(세션 시작 시 자동으로
   확인·인터뷰).
2. **Motto of the week / Urgent items 표(메인 표)** — 요일별로 한 행씩:
   - 요일 라벨 칸 (`Sun`, `Mon`, ...)
...
날짜가 들어가는 자리는 **메인 표(2번) 안의 7개 요일 행뿐**이다. 1번 표는
항상 빈 칸(항목 라벨만 있고 값은 비어있음)으로 둔다.
...
1. 템플렛 노트의 `get_note` 결과에서 `content` 전체(2개 표 + 구분 div)를 그대로
   가져온다.
...
## 6.1 요일별 할일 순서 정렬 (2026-09-06부터)

각 요일 칸의 할일 리스트(체크박스 항목들)는 **항상 시간 순서대로 정렬**한다 —
새 주 노트를 만들 때뿐 아니라, 그 주가 진행되며 이메일 스캔 등으로 항목을
추가·수정할 때도 매번 그 요일의 리스트 전체를 다시 정렬한다. 규칙:

1. **시간이 명시된 항목**은 24시간 기준 오름차순으로 정렬한다 (예: "양자계산과학
   수업 12:00 - 1:30" → 12:00, "학과장 회의 1pm" → 13:00).
2. **`퇴근: <시각>` 항목의 시각은 항상 오후(PM)로 해석한다** — "퇴근: 8시"는
   20:00, "퇴근: 4:30"은 16:30, "퇴근: 11시"는 23:00. (오전 퇴근은 없다.)
3. **시간이 전혀 없는 일반 할일**(예: "K10 YR 쓰기", "HIN-22-003", "오전 학과
   회의 준비", "JPS 발표 준비 (1/3)"처럼 준비/작성류)은 시간이 있는 항목들보다
   **앞쪽**에 몰아서 두고, 그들끼리는 원래 순서를 유지한다 — 특정 시각에 매이지
   않고 하루 중 아무 때나 할 수 있는 일이기 때문이다. ("오전"이라고만 적혀 있어도
   구체적 시:분이 없으면 이 그룹에 속한다.)
4. **시간이 없지만 "마감"류 항목**(예: "[메일] ... 신청 마감", "... 제출 마감")은
   반대로 **맨 뒤**(하루가 끝나기 전까지)에 둔다 — 데드라인은 암묵적으로
   "그날 안으로만 하면 됨"이라는 뜻이라 사실상 그 날의 가장 늦은 시각으로
   취급한다.
5. 정리하면 한 요일의 최종 순서는:
   `[시간 없는 일반 할일들(원래 순서)] → [시간 있는 항목들(오름차순, 퇴근은 PM)] → [시간 없는 마감류(원래 순서)]`

## 6.2 반복 일정 변경 반영 (요일/시각이 바뀐 경우)

사용자가 "그 미팅 이제 요일/시각 바뀌었어" 처럼 기존 반복 일정의 요일이나
시각 변경을 알려주면:

1. **템플렛 노트를 고친다** — 반복 일정의 실제 내용(회의명·요일·시각)은
   Evernote 템플렛 노트 안에만 있는 데이터이므로, 변경 사항은 그 템플렛
   노트의 해당 요일 셀을 `edit_note(mode:"replace")`로 옮겨서 반영한다.
   기존 요일 셀에서 항목을 제거하고 새 요일 셀에 6.1절 정렬 규칙에 맞는
   위치로 삽입한다.
2. **이미 만들어진 이번 주 노트도, 아직 지나지 않은 날짜라면 같이 고친다**
   (예: 변경 시점 기준 이번 주의 새 요일이 아직 안 지났으면). 이미 지난
   요일은 소급 수정하지 않는다.
3. **이 SKILL.md에는 실제 회의명이나 요일/시각을 적지 않는다** — 11절 개인정보
   규칙 그대로, 템플렛 노트를 고치는 것만으로 다음 주부터 자동 반영된다.
   금요일 자동 routine도 매번 템플렛 노트를 새로 읽어오므로(8절), 템플렛만
   고치면 routine 프롬프트를 별도로 갱신할 필요가 없다 — routine 프롬프트
   자체가 이 절차(SKILL.md)를 그대로 인라인한 것이므로 절차가 바뀐 게
   아니라면 동기화가 필요 없다.
