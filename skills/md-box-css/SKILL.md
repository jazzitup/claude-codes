...
## 4. Obsidian 라이트/다크 테마 모두 대응하는 3중 정의 패턴을 쓴다
...
- **주차마다 "과학사 한 장면" 섹션의 원래 형태가 다를 수 있다** (2026-09-07,
  같은 강의의 Week02는 이미 `<div class="histbox">` 구조로 되어 있었지만
  Week03~05는 div가 전혀 없이 그냥 `![사진]()` + 이름/연도 + 평문
  "과학사 한 장면 — 제목" + 문단들로만 되어 있었다). div가 없는 경우엔
  "펴서 다시 압축"할 게 아니라 **처음부터 한 줄짜리 histbox HTML로
  새로 감싸야** 한다 — 이때도 경계(어디까지가 이 박스의 마지막 문단인지)를
  정확히 잡아야 하는데, 보통 다음 중 하나가 나오면 그 직전 빈 줄이 끝이다:
  다음 `###`/`##` 제목, `---` 구분선, `$$` 수식 블록, 다른 이미지, 또는
  다시 본문(연습문제 등)으로 돌아가는 문장. `grep -n '과학사 한 장면\|^!\['`
  로 위치를 잡고 앞뒤 30~40줄을 읽어서 직접 경계를 확인한다.
- 이미지 alt 텍스트가 비어있는 경우(`![]()`)가 있다 — 이때는 바로 아래
  줄에 있는 이름 텍스트를 alt로 대신 채워 넣는다(접근성 + `<img alt>`
  누락 방지).
- 정규식으로 이모지스(`emphasis`) 변환할 때 `[^*\n]`처럼 줄바꿈을
  제외하면 안 된다 — 문단이 80자 안팎에서 줄바꿈되어 있어서 `*이탤릭
  단어*`가 문장 중간에 줄바꿈을 사이에 두고 걸치는 경우가 실제로
  있었다(2026-09-07, Week04의 저널명 "*Physics Physique\nFizika*").
  `[^*]`(줄바꿈 포함 허용)를 쓰고, 치환 후 각 박스 안에 `*...*`가
  그대로 남아있지 않은지 정규식으로 스캔해서 검증한다.
