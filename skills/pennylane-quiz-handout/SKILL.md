...
마크다운에서 언더스코어를 여러 개 연속으로 쓰면(`______________________`)
일부 렌더러(이 사용자가 인쇄 미리보기에 쓰는 렌더러 포함, 2026-09-01에
실제로 겪음)가 이를 강조(emphasis) 토글로 해석해서 서로 짝을 지어
없애버리고, 인쇄했을 때 밑줄이 2~3칸만 남는 사고가 난다. **반드시 각
언더스코어를 `\_`로 이스케이프**해서 리터럴 문자로 렌더링되게 한다
(위 3절 템플릿 예시가 이미 그렇게 되어 있다). 이름/학번 빈칸 길이는
각각 30자 이상(이스케이프된 언더스코어 기준)으로 충분히 길게 잡는다 —
손글씨로 채울 칸이라 짧으면 다시 고쳐야 한다.
