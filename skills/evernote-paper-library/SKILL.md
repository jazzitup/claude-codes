...
- **논문 라이브러리 노트북은 하나가 아닐 수 있다.** Babel 노트의 여러
  섹션(A~J)이 서로 다른 notebookId를 가리키고 있을 수 있다(과거에 쓰던
  노트북과 지금 쓰는 노트북이 섞여 있는 등). 새 논문 노트를 만들 때는
  "그 섹션에 최근에 추가된 예시 노트"가 실제로 속한 notebookId를 쓰면
  된다 — 라이브러리 전체를 하나의 notebookId로 통일하려 하지 않는다.
- **Babel 노트는 사용자가 실시간으로 직접 편집하기도 한다.** 섹션
  이름·서브섹션 이름이 스킬 실행 중에도 바뀔 수 있고, `get_note`로 읽은
  직후 `edit_note(mode:"replace")`의 `find`가 어긋날 수 있다(사용자가
  그새 편집했기 때문). `find`가 매치 안 되면 즉시 Babel 노트를 다시
  `get_note`로 새로 읽어서 최신 상태 기준으로 앵커를 다시 뽑아 재시도한다
  — 캐시된/이전 턴의 내용을 재사용하지 않는다.
