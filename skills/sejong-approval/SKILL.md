...
5. **"처리완료 하였습니다!" 네이티브 브라우저 alert가 뜨며 탭 전체가
   멈춘다.** claude-in-chrome에는 이 다이얼로그를 닫는 도구가 전혀 없다
   (`handle_dialog` 같은 게 없음) — alert가 떠 있는 동안 그 탭에 대한
   모든 `javascript_tool`/`computer screenshot` 호출은 "Script injection
   timed out" 에러로 실패한다. **미리 `window.alert`를 오버라이드해서
   막으려는 시도도 위 3번과 같은 이유로 권한 분류기에 막힌다** — 이
   단계는 자동화가 불가능하다. 반드시 사용자에게 "확인/OK를 눌러
   주세요"라고 요청하고, 사용자가 눌렀다고 확인해줄 때까지 기다린 뒤
   `computer screenshot`을 재시도한다. 이걸 승인마다 매번 반복해야
   하므로, 문서가 여러 건이면 사용자에게 이 점을 미리 안내한다
   ("문서 N건마다 완료 alert를 직접 닫아주셔야 합니다").
6. alert가 닫히면 문서 화면의 툴바에서 "결재"/"문서수정" 버튼이 사라지고
   "공람지정"만 남는 것으로 처리 완료를 확인할 수 있다.
7. `mcp__claude-in-chrome__navigate`로 `https://sjgw.sejong.ac.kr/xclick_sju/main.jsp`
   로 되돌아간 뒤(만든 iframe은 이 탐색으로 자동 정리된다) 전자결재 메뉴를
   다시 클릭해 결재대기함을 다시 확인한다. 대시보드의 "미결문서 (N)"
   카운트가 줄었는지, 미결문서 위젯에서 방금 처리한 항목이 빠졌는지로
   확인한다.
8. 결재대기함이 빌 때까지 1~7을 반복한다.
